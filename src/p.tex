...
集合族に対しても, 合併集合\footnote{$A\cup B$のことを和集合と呼ぶこともあるが, 和空間との混同を避けるため, ここでは合併集合という語を用いる.}や共通部分を考えることもある.
\begin{definition}
$\XXX$を集合族とする.
...
$\alpha,\beta\in\KK, w\in V\implies (\alpha+\beta)\act w=(\alpha\act w)\plus (\beta\act w)$.
  ...
  \end{enumerate}
体$\KK$上の線形空間のことを
$\KK$上の\defit{ベクトル空間}とか$\KK$-ベクトル空間とも呼ぶ.
\end{definition}
\begin{remark}
...
\end{prop}
\begin{proof}
\paragraph{\Cref{prop:isoisequiv:1}}
$\id_V\colon V\to V$は$V$から$V$への線形写像であり,
$\id_V\circ \id_V=\id_V$であるので,
$V$から$V$への同型写像である.

\paragraph{\Cref{prop:isoisequiv:2}}
...
\paragraph{\Cref{prop:isoisequiv:3}}
$\varphi\colon V\to W$,
$\varphi'\colon W\to U$
が同型写像であるとする.
$\psi$を
...
\end{example}
\begin{proof}
\Cref{prop:zeromapislinear,prop:sumislinear,prop:scalarprodislinear}
からわかる.
\end{proof}
...
したがって, \Cref{prop:homisvecsp}より$V^\ast=\Hom_\KK(V,\KK)$は,
$\KK^V$の部分空間である.
\end{proof}
...
\begin{example}
  ...
  $\RR\subset \CC$ではあるが,
  `$\KK$'が異なるので,
  $\RR$-線形空間$\RR$は$\CC$-線形空間$\CC$の部分空間であるとは言わない.
...
$\KK$線形空間
$(V,\plus,\act,0_V)$に対し,
零ベクトルのみからなる列$(0_V)$を考える.
このとき, $\nu_{(0_V)}$は, $\KK^1$から$V$への線形写像であり,
$(a)\in \KK^1$に対し, $\nu_{(0_V)}((a))=a0_V=0_V$となる.
したがって, 単車でないので,
$(0_V)$は一次従属である.
\end{remark}
...
\end{remark}
\begin{remark}
$\KK$線形空間$(V,\plus,\act,0_V)$に対し,
$\Set{0_V}$は$V$の部分空間であった.
空列$()$を考えると,
$\nu_{()}$は$\Set{\zzero_0}$から$V$への線形写像であり,
$\nu_{()}(\zzero_0)=0_V$であるから,
$\Img(\nu_{()})=\Set{0_V}$であると約束した.
したがって, 空列$()$は$\Set{0_V}$の生成系であると約束する.
\end{remark}
\begin{remark}
$\KK$線形空間$(V,\plus,\act,0_V)$に対し,
$0_V$のみからなる列$(0_V)$を考えると,
$\nu_{(0_V)}$は$\KK$から$V$への線形写像であり,
$(a)\in\KK$に対し$\nu_{(0_V)}(a)=a0_V=0_V$を満たすので,
$\Img(\nu_{(0_V)})=\Set{0_V}$である.
したがって, $(0_V)$も$\Set{0_V}$の生成系である.
\end{remark}
...
$\KK$線形空間$(V,\plus,\act,0_V)$に対し,
$\Set{0_V}$は$V$の部分空間であった.
空列$()$は一次独立であり, $\Set{0_V}$の生成系であるので,
$\Set{0_V}$の基底であると約束する.
\end{remark}
\begin{remark}
$\KK$線形空間$(V,\plus,\act,0_V)$に対し,
 $0_V$のみからなる列$(0_V)$を考えると,
$(0_V)$も$\Set{0_V}$の生成系であった.
しかし, $(0_V)$は一次従属であるので,
$\Set{0_V}$の基底ではない.
\end{remark}
...
とし,
\begin{align*}
\shazo{\mu_A}{\RR^7}{\RR^5}
{\xx}{A\xx}
\end{align*}
を考える.
\begin{enumerate}
\item $\Ker(\mu_A)$の基底を1組挙げよ.
\item $\dim_\RR(\Ker(\mu_A))$を求めよ.
\item $\Img(\mu_A)$の基底を1組挙げよ.
\item $\dim_\RR(\Img(\mu_A))$を求めよ.
\item $\dim_\RR(\Img(\mu_A))+\dim_\RR(\Ker(\mu_A))$を求めよ.
\end{enumerate}
\end{quiz}
...
\end{lemma}
\begin{proof}
\paragraph{\Cref{item:quotientclass:1}$\implies$\Cref{item:quotientclass:2}.}
$u=u+0_V\in [u]_V = [u']_V$であるので,
$u\in [u']_V$. よって, $u=v\plus u'$を満たす$v\in V$がとれる.
このとき, $u\plus (-u')=v\in V$.

\paragraph{\Cref{item:quotientclass:2}$\implies$\Cref{item:quotientclass:1}.}
