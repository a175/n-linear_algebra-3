...
\begin{definition}
  ...
  \item
    \begin{enumerate}
    \item $a\in \KK,u,w\in V \implies a\act (u\plus w)=(a\act u)\plus (a\act w)$.
    \item $a,b\in\KK, w\in V\implies (a+b)\act w=(a\act w)\plus (b\act w)$.
    \end{enumerate}
  \end{enumerate}
\end{definition}
\begin{remark}
...
\begin{remark}
ベクトル空間の元のことをベクトルと呼ぶ.
ベクトルとは向きと大きさを持ったものと説明されることもあるが,
...
\begin{example}
  ...
  \end{pmatrix}\in\KK^n
  \end{align*}
  は, $a=a_1\ee^{(n)}_i+\cdots+a_n\ee^{(n)}_n$
  と書くことができる.
...
\begin{prop}
  ...
  \begin{align*}
    w\in V \implies w\plus o=w.
  \end{align*}
  このとき, $o=0_V$.
\end{prop}
\begin{proof}
  $0_V$の満たす条件から$0_V\plus o=o$.
  一方, $o$の満たす条件から$0_V\plus o=0_V$.
  よって, $0_V=0_V\plus o=o$.
\end{proof}
\begin{remark}
  $(V,\plus,\act,0_V)$を$\KK$線形空間とする.
  $u,w\in V$に対し, $u\plus w=w\plus u$であるので,
  次の条件は同値である:
  \begin{enumerate}
  \item $w\in V \implies w\plus o=w$.
...
$(V,\plus,\act,0_V)$を$\KK$線形空間とする.
  $x\in x$は
  $0\act w\plus x=0_V$を満たすとする.
  このとき,
  \begin{align*}
    (0\act w\plus 0\act w)\plus x&=(0+0)\act w\plus x=0\act w\plus x=0_V\\
    0\act w\plus (0\act w\plus x)&=0\act w\plus 0_V=0\act w
  \end{align*}
  よって, $0\act w=0_V$.  
\end{proof}
...
\end{prop}
\begin{proof}
$c\act 0_V = c\act (0\act 0_V)= (c\cdot 0)\act 0_V=0\act 0_V=0_V.$
\end{proof}
...
\begin{remark}
  ...
  条件をまとめて,
  \begin{enumerate}
    \item $v,v'\in V, c,c'\in\KK\implies \varphi(c\act v\plus c'\act v')=c\aact \varphi(v)\pplus c'\aact\varphi(u)$
  \end{enumerate}
  を満たすことととして定義することもある.
\end{remark}
...
\begin{example}
  $V$
  $(W,\pplus,\aact,\zzero_W)$
  を$\KK$線形空間とする.
    \begin{align*}
      \shazo{\underline{\zzero_W}}{V}{W}
      {x}{\zzero_W}
    \end{align*}
  は$\KK$-線形写像である.
\end{example}
...
\begin{example}
  $V$, $(W,\pplus,\aact,\zzero_W)$を$\KK$-線形空間とし,
...
\begin{example}
  ...
  \end{align*}
  について考える.
  $\rank(A)\leq n<m$であるので, $\varphi$は全射ではない.
  したがって,
...
は成り立つ.
同様の理由で,
$a,b\in \KK$, $v,w\in W$に対して,
\begin{align*}
  1\act v&=v\\
  (ab)\act v&=a\act(b\act v)\\
  (a+ b)\act v&=a\act v\plus b\act v\\
  a\act(v\plus w)&=a\act v\plus a\act w
\end{align*}
が成り立つ.
...
\begin{enumerate}
  ...
  \item $a\in\KK$, $v\in W \implies a\act v\in W$.
\end{enumerate}
\end{remark}
\begin{lemma}
  $(V,\plus ,\act,0_V)$を$\KK$-線形空間とし, $W\subset V$とする.
  ...
  \item 以下の$2$条件を満たす:
    \begin{enumerate}
  \item $0_V\in W$.
  \item $v,w\in W, a,b\in\KK \implies a\act v\plus b\act w\in W$.
    \end{enumerate}
  \item 以下の$3$条件を満たす:
    \begin{enumerate}
  \item $W\neq \emptyset$.
...
\begin{proof}
  ...
  これらは, $V\cap W$の元.
  よって, $\zzero$である.
  $-v'\plus v=\zzero$であるので, $v=v'$.
  $-w'\plus w=\zzero$であるので, $w=w'$.
...
\begin{example}
  ...
  は$\KK$線形写像.
  \begin{align*}
    \Ker(\mu_A)=\Set{x\in\KK^n|Ax=\zzero_m}
  \end{align*}
...
\begin{definition}
  ...
  \begin{enumerate}
  \item $c_1\act w_1\plus\cdots\plus c_r\act w_r=\zzero \implies c_1=\cdots=c_r=0$.
  \end{enumerate}
...
\begin{prop}
  $w_i\in V$とし, 
  $(w_1,\ldots,w_r)$は$\KK$上一次独立であるとする.
  \begin{align*}
  v&=a_1\act w_1\plus\cdots \plus a_r\act w_r\\
  v'&=a'_1\act w_1\plus\cdots \plus a'_r\act w_r\\
  \end{align*}
  とする.
  このとき以下は同値:
  \begin{enumerate}
  \item $v=v'$.
  \item すべての$i$に対し, $a_i=a'_i$.
  \end{enumerate}
\end{prop}
\begin{proof}
\begin{align*}
...
\begin{definition}
  ...
  \item
    ...
    \begin{enumerate}
    \item
    $c_1\act w_1\plus \cdots c_r\act w_r=v$
...
\begin{prop}
  ...
  つまり,
  $v=a_1\act w_1\plus \ldots\plus a_r\act w_r$
  を満たす$a_1,\ldots, a_r\in\KK$がただ一組存在する.
\end{prop}
...
\end{prop}
\begin{remark}
\Cref{prop:basis:ext}で得られる$W$の基底
...
$V$を$\KK$線形空間とする.
  ...
  このとき,
  $1\leq i_1<\cdots<i_k \leq r$
  $(v_{i_1},\ldots, v_{i_k})$は$\KK$上一次独立.
\end{prop}
...
\begin{remark}
  ...
  \begin{align*}
    ...
    c\act [u]_V&=[c\act u]_V=\Set{c\act u|v\in V}
  \end{align*}
  とする.
\end{remark}
...
\begin{enumerate}
  ...
  \item $c\varphi+\varphi'$の$B$, $D$に関する表現行列は$cF$.
...
\begin{remark}
  $D=(v_1,\ldots,v_n)$を$V$の基底とすると,
  $\mu_D$は同型写像であり,
  $D=(\mu_D(\ee^{(n)}_1)),\ldots,\mu_D(\ee^{(n)}_1)))$である.
  逆に, 同型写像$\varphi\colon \KK^n\to V$が与えられたとき,
  $D=(\varphi(\ee^{(n)}_1)),\ldots,\varphi(\ee^{(n)}_1)))$は$V$の基底である.
  したがって,
  $V$の基底を与えることと同型写像$\varphi\colon \KK^n\to V$は同じことである.
...
\begin{definition}
  ...
  \item $v_1,\ldots,v_n\in V$, $c\in\KK$に対し,
    \begin{align*}
      F(v_1,\ldots,v_{i-1},c\act v_i,v_{i+1},\ldots,v_n)=c\aact F(v_1,\ldots,v_{i-1},v_i,v_{i+1},\ldots,v_n).
    \end{align*}
  \end{enumerate}
\end{definition}
\begin{remark}
...
\begin{remark}
  ...
  \begin{align*}
    E(\varphi,\lambda) &= \Ker( \lambda\id_V - \varphi)\\
    &= \Set{v\in V|(\lambda\id_V-\varphi)(v)=\zzero}\\
    &= \Set{v\in V|\lambda v-\varphi(v)  =\zzero}\\
...
\begin{definition}
  ...
  $\lambda \in \KK$に対し,
  \begin{align*}
    E(\varphi,\lambda) = \Set{v\in\KK^n|(\lambda E_n-A)v=\zzero_n}
  \end{align*}
  とおく.
  $E(\varphi,\lambda)\neq \Set{\zzero}$であるとき,
  \begin{enumerate}
